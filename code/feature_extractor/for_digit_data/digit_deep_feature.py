...
def finetune(model, dataloaders, optimizer):
    ...
    for epoch in range(1, N_EPOCH + 1):
        for phase in ['src', 'val', 'tar']:
            ...
            if phase == 'val' and epoch_acc > best_acc:
                ...
                best_model_wts = copy.deepcopy(model.state_dict())
                torch.save(model.state_dict(), os.path.join(setting.MODEL_ROOT, 'best_{}-{}-{}.pth'.format(args.source, args.target, epoch)))
                np.savetxt(os.path.join(setting.LOG_ROOT, '{}_{}_hist.csv'.format(args.source, args.target)), np.asarray(acc_hist, dtype=float),
                           fmt='%.6f', delimiter=',')
        print()
    ...
    torch.save(model.state_dict(), os.path.join(setting.MODEL_ROOT, 'best_{}_{}.pth'.format(args.source, args.target)))
...
if __name__ == '__main__':
    ...
    root_dir = {'src':os.path.join(base_path,"data","mnist"),'tar':os.path.join(base_path,"data","usps")}
    ...
    model_path = os.path.join(setting.MODEL_ROOT, 'best_{}_{}.pth'.format(args.source, args.target))
