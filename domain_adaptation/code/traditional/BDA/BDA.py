...
#calculate mu depend on its proxy a distance
def estimate_mu(_X1, _Y1, _X2, _Y2):
    ...
    for i in range(1, C + 1):
        ind_i, ind_j = np.where(_Y1 == i), np.where(_Y2 == i) #indices of class i
        Xsi = _X1[ind_i[0], :]
        Xtj = _X2[ind_j[0], :]
...
class BDA:
    ...
    def fit_predict(self, Xs, Ys, Xt, Yt):
        ...
        for t in range(self.T):
            ...
            K = utils.kernel(self.kernel_type, X, None, gamma=self.gamma)
...
if __name__ == '__main__':
    ...
    data_root = os.path.join(setting.DATA_ROOT, 'data_mat')


    for i in range(1):
        for j in range(2):
            if i != j:
                src, tar = os.path.join(data_root,domains[i]), os.path.join(data_root,domains[j])
