...
def load_mnist(path, scale=True, usps=False, all_use=True):
    ...
    if scale:
        mnist_train = np.reshape(mnist_data['train_32'][:55000], (55000, 32, 32, 1))
        mnist_test = np.reshape(mnist_data['test_32'], (10000, 32, 32, 1))
        mnist_train = np.concatenate([mnist_train, mnist_train, mnist_train], 3) #copy chanel
        mnist_test = np.concatenate([mnist_test, mnist_test, mnist_test], 3)
        mnist_train = mnist_train.transpose(0, 3, 1, 2).astype(np.float32)   #num_sample * chanel_num * width * height
        mnist_test = mnist_test.transpose(0, 3, 1, 2).astype(np.float32)
        mnist_labels_train = mnist_data['label_train'][:55000]
        mnist_labels_test = mnist_data['label_test']
    else:
        mnist_train = np.reshape(mnist_data['train_28'][:55000],(-1, 28,28, 1))
        mnist_test = np.reshape(mnist_data['test_28'],(-1, 28,28, 1))
        mnist_labels_train = mnist_data['label_train'][:55000]
        mnist_labels_test = mnist_data['label_test']
    ...
    train_label = mnist_labels_train
    inds = np.random.permutation(mnist_train.shape[0])
    mnist_train = mnist_train[inds]
    train_label = np.squeeze(train_label)[inds]
    #test_label = np.argmax(mnist_labels_test, axis=1)
    test_label = np.squeeze(mnist_labels_test)
...
def load_usps(path, all_use=True, type = "mat"):
    if type == 'pkl':
        f = gzip.open(path, 'rb')
        ...
        usps_mat = loadmat(path)
        img_train = usps_mat['train_28']
        img_test = usps_mat['test_28']
        label_train = np.squeeze(usps_mat['label_train'])
        label_test = np.squeeze(usps_mat['label_test'])
    else:
        raise ValueError("Don't support file type!")
...
def load_dataset(domain, root_dir):
    ...
    if domain == 'mnist':
        train_img, train_label, test_img, test_label = load_mnist(os.path.join(root_dir, 'mnist.mat'),all_use=False)
    if domain == 'usps':
        train_img, train_label, test_img, test_label = load_usps(os.path.join(root_dir,'usps.mat'),all_use=False)
    if domain == 'svhn':
        train_img, train_label, test_img, test_label = load_svhn(os.path.join(root_dir,'svhn_train_32x32.mat'),
                                                                 os.path.join(root_dir + 'svhn_test_32x32.mat'))
    return train_img, train_label, test_img, test_label


def load_data(domain, root_dir, batch_size):
    src_train_img, src_train_label, src_test_img, src_test_label = load_dataset(domain['src'], root_dir['src'])
    tar_train_img, tar_train_label, tar_test_img, tar_test_label = load_dataset(domain['tar'], root_dir['tar'])
